...
@Component({
  ...
  templateUrl: './choose-panel.page.html',
  styleUrls: ['./choose-panel.page.scss'],
...
export class ChoosePage implements OnInit, OnDestroy {
  ...
  setSelectedUser(user) {
    ...
    if (this._router.url !== '/choose' && this._router.url !== '/activity-summary') {
  ...
  setNotifications() {
    // Get Module
    const pageIndex = this._notificationService.appIconPages.findIndex(x => x['title'] == 'Choose');
    if (pageIndex !== -1) {
