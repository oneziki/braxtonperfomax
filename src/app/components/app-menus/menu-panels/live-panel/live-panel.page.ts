...
@Component({
  ...
  templateUrl: './live-panel.page.html',
  styleUrls: ['./live-panel.page.scss'],
  encapsulation: ViewEncapsulation.None
})
export class LivePanel implements OnInit, OnDestroy {
  ...
  setPerformUser(user) {
    ...
    if (this._router.url !== '/live' && this._router.url !== '/activity-summary') {
  ...
  setNotifications() {
    // Get Module
    const pageIndex = this._notificationService.appIconPages.findIndex(x => x['title'] == 'Live');
    if (pageIndex !== -1) {
