...
RowsGroup.prototype = {
    ...
    for (i = 0; i < columnsIndexes.length; ++i) {
      ...
      for (iRow = iStartRow + 1; iRow <= iFinishRow; ++iRow) {
        if (columnValues[iRow] === columnValues[newSequenceRow] || (columnValues[iRow]['label'] && (columnValues[iRow]['label'] === columnValues[newSequenceRow]['label']) )) {
          // console.log('~~', iRow, newSequenceRow,'IF:: ', ((iRow - 1) - newSequenceRow + 1), columnValues[newSequenceRow], ' === ', columnValues[iRow]);
          $(columnNodes[iRow]).hide()
        } else {
          // console.log('~~', iRow, newSequenceRow,'ELSE:: ', ((iRow - 1) - newSequenceRow + 1), columnValues[newSequenceRow], ' === ', columnValues[iRow]);
          if (newSequenceRow === 1) {
            // $(columnNodes[iRow-1]).hide()
          ...
          if (columnsIndexesCopy.length > 0) this._mergeColumn(columnsIndexes[i], (iRow-1), columnsIndexesCopy)
          newSequenceRow = iRow;
        }
